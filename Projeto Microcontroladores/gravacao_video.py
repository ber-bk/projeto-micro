        ...
        try:
            ...
            while not self.parar_flag:
                ...
                while not self.parar_flag:
                    ...
                    texto_info = f"Missao: {self.identificador_missao} | Seg: {segmento_numero}"
                    texto_tempo = f"Tempo: {int(tempo_decorrido)}s / {DURACAO_SEGMENTO}s"

                    # Obter dados dos sensores
                    sensor = sensor_arduino.get_sensor()
                    temperatura = sensor.get_temperatura_formatada()
                    pressao = sensor.get_pressao_formatada()
                    texto_sensores = f"Temp: {temperatura} | Pressao: {pressao}"

                    cv2.putText(frame, texto_info, (10, 30),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                    cv2.putText(frame, texto_tempo, (10, 60),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                    cv2.putText(frame, texto_sensores, (10, 90),
